...
"""
    使用位置
    requests.get(xxx, auth=HTTPSKerberosAuth())
"""
...
def _get_channel_bindings_application_data(response):
    ...
    try:
        ...
        try:
            ...
            # _get_certificate_hash 暂时删除
            pass
    return application_data


class HTTPSKerberosAuth(AuthBase):
    """Attaches HTTPS GSSAPI/Kerberos Authentication to the given Request
    object."""

    def __init__(
            self, mutual_authentication=REQUIRED,
            service="HTTPS", delegate=False, force_preemptive=False,
    ...
    def handle_other(self, response):
        ...
        if self.mutual_authentication in (REQUIRED, OPTIONAL) and not self.auth_done:
            ...
            log.debug(f"handle_other(): returning {0}".format(response))
            return response
